import { requestAction } from '../ai/client';
import type { ActionRequest, PetState } from '../lib/types';
...
export function startAiLoop({ state, fsm, onThought }: AiLoopOptions): AiLoopHandle {
  let stopped = false;
  ...
  const fire = async () => {
    ...
    inflight = true;
    try {
      const reply = await requestAction(buildRequest(state, fsm));
      if (stopped) return;
      const intent = {
